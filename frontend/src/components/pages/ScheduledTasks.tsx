import { Play, RefreshCw, Clock, CheckCircle, XCircle, AlertCircle, Loader, RotateCw } from 'lucide-react';
import { useTriggerDailyRunner, useDailyRunnerStatus, useDailyRunnerHistory, useRetryExecution } from '../../hooks/useJobs';
...
const ScheduledTasks: React.FC = () => {
  ...
  // 移除自动刷新，改为手动刷新
  const { status, loading: statusLoading, refetch: refetchStatus } = useDailyRunnerStatus(undefined, false);
  const { history, loading: historyLoading, refetch: refetchHistory } = useDailyRunnerHistory(undefined, undefined, 20);
  const { retry: retryExecution, loading: retryLoading } = useRetryExecution();

  const handleRetry = async (execution_id: string) => {
    try {
      const result = await retryExecution(execution_id);
      if (result) {
        showToast(`重试任务已触发，执行ID: ${result.execution_id}`, 'success');
        setTimeout(() => {
          refetchStatus();
          refetchHistory();
        }, 1000);
      ...
      const apiError = error as { 
        response?: { 
          data?: { 
            detail?: { error?: string; message?: string; user_friendly?: boolean };
            error?: string;
            message?: string;
          } 
        };
  ...
  const handleTrigger = async (force: boolean = false) => {
    try {
      ...
      const apiError = error as { 
        response?: { 
          data?: { 
            detail?: { error?: string; message?: string; user_friendly?: boolean; hint?: string };
            error?: string;
            message?: string;
          } 
        };
      ...
      // 优先检查detail字段（从API响应或错误对象）
      const errorDetail = apiError?.response?.data?.detail || apiError?.detail;
      
      if (errorDetail) {
        // 如果是重复执行错误，显示更友好的提示
        if (errorDetail.error === 'DUPLICATE_EXECUTION' || 
            (errorDetail.user_friendly && errorDetail.message?.includes('已有成功执行记录'))) {
          const friendlyMsg = errorDetail.message || '该交易日期已有成功执行记录';
          showToast(
            `${friendlyMsg}。如需重新执行，请点击"强制执行"按钮。`,
            'warning',
            5000  // 显示5秒，让用户有时间阅读
          );
          return;
        ...
        // 其他错误，使用API返回的消息
        const errorMsg = errorDetail.message || errorDetail.hint || '触发任务失败';
        showToast(`触发失败: ${errorMsg}`, 'error');
        return;
      }
      
      // 检查response.data中的message（标准API错误格式）
      if (apiError?.response?.data?.message) {
        const errorMsg = apiError.response.data.message;
        // 检查是否是重复执行相关的消息
        if (errorMsg.includes('已有成功执行记录') || errorMsg.includes('DUPLICATE')) {
          showToast(
            `${errorMsg}。如需重新执行，请点击"强制执行"按钮。`,
            'warning',
            5000
          );
          return;
  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">定时任务管理</h1>
                ...
                <tbody>
                  {history.executions.map((exec: ExecutionStatus) => (
                    <tr key={exec.execution_id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        {(exec.status === 'FAILED' || exec.status === 'TIMEOUT') && exec.retry_count < exec.max_retries && (
                          <button
                            type="button"
                            onClick={() => handleRetry(exec.execution_id)}
                            disabled={retryLoading}
                            className="flex items-center gap-1 px-2 py-1 text-xs bg-yellow-100 text-yellow-700 rounded hover:bg-yellow-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            title="手动重试此任务"
                          >
